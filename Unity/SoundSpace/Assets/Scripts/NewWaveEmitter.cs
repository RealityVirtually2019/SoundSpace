...
public class NewWaveEmitter : MonoBehaviour
{
    ...
    private RaycastHit[] Hits; //Store hit results
    private bool[] isHit;
        ...
        if (hasBeenPressed == 1)
        {
            ...
            for (int m = 0; m < numOfVectors; m++)
            {
                ...
                if(isHit[m])
                {
                    ...
                    if (Vector3.Distance(OldPosition, hit.point) < stepFactor)
                    {
                                             
                        Vector3 Deconstruct = 0.05f * hit.normal;                       
                        ...
                        RaycastHit newhit;
                        isHit[m] = Physics.Raycast(OldPosition, TravelVectors[m], out newhit, 1000, layerMask);
                        if (isHit[m])
                            Hits[m] = newhit;
    ...
    public void MakeNoise()
    {
        ...
        Hits = new RaycastHit[numOfVectors];
        isHit = new bool[numOfVectors];
        for (int i = 0; i < numOfVectors; i++)
        {
            ...
            if (Physics.Raycast(gameObject.transform.position, TravelVectors[i], out hit, 1000, layerMask))
            {
                isHit[i] = true;
                Hits[i] = hit;
            }
